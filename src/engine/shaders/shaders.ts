...
precision highp float;
in vec2 l;in float f;in vec3 n;in mat4 o;in vec4 u;in vec3 e,d;uniform mediump sampler2DArray s;uniform mediump sampler2DShadow z;uniform mediump sampler2D h;vec3 x=normalize(vec3(-.3,.5,-.2));vec4 y=vec4(.2,.2,.2,1);vec2 w=vec2(-150,0),A=vec2(300,2100);out vec4 g;float p(mediump sampler2DShadow v,vec4 f){float i=0.,n=1./4096.;for(int l=-1;l<=1;l++)for(int m=-1;m<=1;m++){vec2 e=vec2(l,m)*n;i+=texture(v,vec3(f.xy+e,f.z-.001));}return i/9.;}float p(float v,float f,float i){return 2.*f/(i+f-v*(i-f));}float p(vec2 v){return fract(sin(dot(v,vec2(12.9898,78.233)))*43758.5453);}float r(vec2 v){vec2 f=floor(v),n=fract(v);n=n*n*(3.-2.*n);return mix(mix(p(f),p(f+vec2(1,0)),n.x),mix(p(f+vec2(0,1)),p(f+1.),n.x),n.y);}void main(){float v=p(z,u*.5+.5);vec3 m=normalize(mat3(o)*n);float i=max(dot(x,m),0.),t=mix(.2*i,i,v);vec3 a=t*vec3(1),c=a+y.xyz,C=clamp(c,vec3(.3),vec3(1));float B=r(d.xz*2.)-.5,D=distance(d.xz,e.xz);vec2 E=(d.xz-w)/A;float F=texture(h,E).x,G=max(D<10.5+B?1.:0.,F),H=1.-abs(G*2.-1.),I=smoothstep(.35,.65,G+B*.35*H);vec4 J=texture(s,vec3(l,f));float K=dot(J,vec4(.299,.587,.114,1));K=(K-.5)*.55+.5;vec3 L=vec3(K);L*=.4;L=mix(L,vec3(.45),.2);vec3 M=mix(L,J.xyz,I);float N=p(gl_FragCoord.z,1.,3e2),O=smoothstep(.4,1.,N);vec3 P=vec3(.7,.9,.5),Q=mix(M.xyz*C,P,O);float R=dot(Q,vec3(.299,.587,.114));vec3 S=mix(vec3(R),Q,I);g=vec4(S,J.w);if(g.w<.2)discard;}`;
...
layout(location=0) in vec3 i;layout(location=1) in float a;layout(location=2) in float B;layout(location=3) in float m;uniform mat4 E;out float L,f;void main(){gl_Position=E*vec4(i,1);gl_PointSize=a;L=B;f=m;}`;

export const skybox_fragment_glsl = `#version 300 es
precision highp float;
uniform mediump sampler2DArray s;uniform mat4 F;uniform float I,Q,O;in vec4 C;out vec4 g;void main(){vec4 v=F*C;vec3 f=normalize(v.xyz/v.w);float m=atan(f.x,f.z),n=asin(clamp(f.y,-1.,1.));vec2 i;i.x=m/6.28+.5;i.y=n/3.14+.5;vec4 l=texture(s,vec3(i,Q)),L=texture(s,vec3(i,O));g=mix(l,L,I);}`;
