...
precision highp float;
in vec2 l;in float f;in vec3 n;in mat4 o;in vec4 u;in vec3 e,d;uniform mediump sampler2DArray s;uniform mediump sampler2DShadow z;uniform mediump sampler2D h;vec3 x=normalize(vec3(-.3,.5,-.2));vec4 y=vec4(.2,.2,.2,1);vec2 a=vec2(-150,0),A=vec2(300,1500);out vec4 g;float p(mediump sampler2DShadow v,vec4 f){float i=0.,n=1./4096.;for(int l=-1;l<=1;l++)for(int m=-1;m<=1;m++){vec2 e=vec2(l,m)*n;i+=texture(v,vec3(f.xy+e,f.z));}return i/9.;}float p(float v,float f,float i){return 2.*f/(i+f-v*(i-f));}float p(vec2 v){return fract(sin(dot(v,vec2(12.9898,78.233)))*43758.5453);}float r(vec2 v){vec2 f=floor(v),n=fract(v);n=n*n*(3.-2.*n);return mix(mix(p(f),p(f+vec2(1,0)),n.x),mix(p(f+vec2(0,1)),p(f+1.),n.x),n.y);}void main(){float v=p(z,u*.5+.5);vec3 m=normalize(mat3(o)*n);float i=max(dot(x,m),0.),t=f<2.?1.:mix(.2*i,i,v);vec3 c=t*vec3(1),C=c+y.xyz,D=clamp(C,vec3(.3),vec3(1));float w=r(d.xz*2.)-.5,B=distance(d.xz,e.xz);vec2 E=(d.xz-a)/A;float F=texture(h,E).x,G=max(B<10.5+w?1.:0.,F),H=1.-abs(G*2.-1.),I=f<2.?1.:smoothstep(.35,.65,G+w*.35*H);vec4 J=texture(s,vec3(l,f));float K=dot(J,vec4(.299,.587,.114,1));K=(K-.5)*.55+.5;vec3 L=vec3(K);L*=.4;L=mix(L,vec3(.45),.9);vec3 M=mix(L,J.xyz,I),N=M.xyz*D;float O=p(gl_FragCoord.z,1.,3e2),P=smoothstep(.8,1.,O),Q=dot(N,vec3(.299,.587,.114));vec3 R=mix(vec3(Q),N,I);g=vec4(R,J.w-P);if(J.w<.2)discard;}`;
