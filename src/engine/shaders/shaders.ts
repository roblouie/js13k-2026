...
precision highp float;
in vec2 l;in float f;in vec3 n;in mat4 o;in vec4 u;in vec3 e,d;uniform mediump sampler2DArray s;uniform mediump sampler2DShadow z;uniform mediump sampler2D h;vec3 x=normalize(vec3(-.3,.5,-.2));vec4 y=vec4(.2,.2,.2,1);vec2 w=vec2(-256),A=vec2(512,1024);out vec4 g;float p(mediump sampler2DShadow v,vec4 f){float i=0.,n=1./4096.;for(int l=-1;l<=1;l++)for(int m=-1;m<=1;m++){vec2 e=vec2(l,m)*n;i+=texture(v,vec3(f.xy+e,f.z-.002));}return i/9.;}float p(float v,float i,float f){return 2.*i/(f+i-v*(f-i));}float p(vec2 v){return fract(sin(dot(v,vec2(12.9898,78.233)))*43758.5453);}float r(vec2 v){vec2 f=floor(v),n=fract(v);n=n*n*(3.-2.*n);return mix(mix(p(f),p(f+vec2(1,0)),n.x),mix(p(f+vec2(0,1)),p(f+1.),n.x),n.y);}void main(){float v=p(z,u);vec3 m=normalize(mat3(o)*n);float i=max(dot(x,m),0.),t=mix(.2*i,i,v);vec3 a=t*vec3(1),c=a+y.xyz,C=clamp(c,vec3(.3),vec3(1));float B=r(d.xz*2.)-.5,D=distance(d.xz,e.xz);vec2 E=(d.xz-w)/A;float F=texture(h,E).x,G=max(D<8.5+B?1.:0.,F),H=1.-abs(G*2.-1.),I=smoothstep(.35,.65,G+B*.35*H);vec4 J=texture(s,vec3(l,f));float K=dot(J,vec4(.299,.587,.114,1));K=(K-.5)*.55+.5;vec3 L=vec3(K);L*=.4;L=mix(L,vec3(.45),.2);vec3 M=mix(L,J.xyz,I);float N=p(gl_FragCoord.z,1.,7e2),O=clamp(smoothstep(.4,1.,N),0.,.5);vec3 P=vec3(.3,.3,.5),Q=mix(M.xyz*C,P,O);float R=dot(Q,vec3(.299,.587,.114));vec3 S=mix(vec3(R),Q,I);g=vec4(S,J.w);if(g.w<.2)discard;}`;

export const particle_fragment_glsl = `#version 300 es
precision highp float;
in float L,f;out vec4 K;uniform mediump sampler2DArray s;void main(){vec4 v=texture(s,vec3(gl_PointCoord,f));float m=v.w*L;if(m<.01)discard;K=vec4(v.xyz,m);}`;
...
layout(location=0) in vec2 F;layout(location=1) in float m;layout(location=2) in vec3 i;layout(location=3) in vec3 J;layout(location=4) in vec3 Q;layout(location=5) in vec3 S;layout(location=6) in vec3 M;layout(location=7) in vec3 N;layout(location=8) in vec3 P;layout(location=9) in vec3 R;uniform vec3 D;uniform mat4 G,I,t,O;uniform float H;uniform int T,U;vec4 V[4];vec3 W[4];out vec2 l;out float f;out vec3 n;out mat4 o;out vec4 u;out vec3 e,d;void main(){V[0]=vec4(i,1);V[1]=vec4(J,1);V[2]=vec4(Q,1);V[3]=vec4(S,1);W[0]=M;W[1]=N;W[2]=P;W[3]=R;vec4 v=mix(V[T],V[U],H);gl_Position=G*v;l=F;f=m;n=mix(W[T],W[U],H);o=I;u=t*v;e=D;d=(O*v).xyz;}`;
